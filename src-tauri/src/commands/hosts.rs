...
pub fn write_hosts_file(state: State<'_, AppState>) -> Result<(), AppError> {
    ...
    // 1. Save all group data (manifest + .hosts files)
    GroupStorageService::save_all(&groups)?;

    // 2. Write active group to system hosts (serialize_groups_clean skips disabled groups)
    let config = state.config.lock().unwrap();
    let hosts_path = config.hosts_path.clone();
    let flush_dns = config.flush_dns_on_save;
    drop(config);

    HostsFileService::write_clean(&groups, Some(&hosts_path))?;

    // 3. Auto flush DNS if enabled
    if flush_dns {
...
pub fn activate_group(
    ...
    std::thread::spawn(move || {
        let _ = HostsFileService::write_clean(&groups_for_bg, Some(&hosts_path));
        if flush_dns {
            let _ = flush_dns_internal();
        }
    });
